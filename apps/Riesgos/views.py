...
from apps.Riesgos.models import poblacion, PerfilDemografico, Riesgo, HombroME, CodoME, CuelloME, ManoME, \
    EspaldaDorsalME, EspaldaBajaME, Cargo, Dependencia, TestME
from apps.Riesgos.formularios import PoblacionForm, PerfilDemograficoForm, HombroMEForm, TestMEForm, CuelloMEForm, \
    ManoMEForm, CodoMEForm, EspaldaDorsalMEForm, EspaldaBajaMEForm
...
def inicio(request):
    return render(request, 'SVE/homepage.html')


def riesgoME(request):
    return render(request, 'RiesgoME/musculoEsqueletico.html')


def enfermedadME(request):
    return render(request, 'RiesgoME/enfermedadesME.html')


def administrarSVE(request):
    return render(request, 'SVE/administrarSVE.html')


def EnviarMensaje(request):
    return render(request, 'RiesgoME/mensajeME.html')
...
def PoblacionCreate(request):
    if request.method == 'POST':
        ...
        form = PoblacionForm()
    return render(request, 'SVE/poblacionSVE.html', {'form': form})


def Poblacionlistar(request):
    Poblacion = poblacion.objects.filter(Riesgo=1)
    contexto = {'poblaciones': Poblacion}
...
def poblacionEnviarCorreo(request):
    ...
    return render(request, 'RiesgoME/enviarCorreoME.html')


def notificaciones(request):
    perfilDemografico = PerfilDemografico.objects.all()
    contexto = {'perfilDemograficos': perfilDemografico}
